  ...
  <form @submit.prevent="confirmPayment()">
    ...
    <div v-if="paymentMethod == 'Bkash'">
      ...
      <input
        ...
        id="phone-number"
        v-model="phoneNumber"
        :class="{ error: !validatePhoneNum && phoneNumber !== '' }"
      />
      <label for="tran-id">Transaction ID</label>
      <input type="text" id="tran-id" v-model="tranID" />
      <p class="error-message" v-if="!validatePhoneNum && phoneNumber !== ''">
        Invalid Phone Number
      </p>
      <p class="error-message" v-if="!validatePresence && submitCount > 0">
        The information is required
      </p>
      <p style="color: rgb(250, 179, 0)">
        Check your information for accuracy. You cannot change it once submitted
      </p>
    </div>
...
export default defineComponent({
  data() {
    return {
      paymentMethod: 'Bkash',
      phoneNumber: pb.authStore.record.Phone_Number,
      tranID: '',
      submitCount: 0
    }
  },
